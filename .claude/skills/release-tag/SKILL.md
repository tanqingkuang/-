...
- **必须使用**：用户明确要求"打 Vx.y.z tag""发布 Vx.y.z""打 tag 并 release"等正式发版动作。
- **禁止**：不要因为"改动看起来可以发版"而主动发起；日常提交、合并 PR、推送分支不属于本 skill 范围。

## 过程输出约束

- `gh` 查询一律用 `--json <字段>` 只取需要的字段，不贴完整 JSON 或网页式输出。
- 等待 GitHub Actions 时只汇报状态变化（排队 / 运行 / 成功 / 失败），失败时才引用失败 job 的关键日志行，不贴全量构建日志。
