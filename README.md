...
| 记录与回归 | 每次运行按 seed 落盘配置、`snapshots_seed_<seed>.jsonl` 和 `events.jsonl`；4 个自动化 ST 场景覆盖直线、编队、避障和十机规模，并维护指标与紧凑轨迹基线。 |
...
7. 每次实际运行的数据保存在 `logs/run-seed-<seed>-*`：`config.json` 是展开后的配置快照，`snapshots_seed_<seed>.jsonl` 是 10 Hz 关键状态，`events.jsonl` 是事件记录。
