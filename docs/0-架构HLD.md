...
- Python 仿真：快速验证算法可行性。
- C 数字仿真：验证 Python 算法移植后的代码。
- 半物理仿真：验证机载硬件与真实链路条件下的可行性。

本文以 Python 仿真为设计对象，同时沉淀可复用于 C 数字仿真和半物理仿真的架构边界、数据契约和模块职责。

# 二、系统PA阶段

| 需求结构化     | 功能                                                         | 质量                                                         | 约束                                                         |
| -------------- | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **业务级需求** | * 支持验证编队的集结、保持、重构（队形变换）和避障的相关能力<br />* 算法快速验证、演示验证、报告生成，并与 C 仿真/半物理仿真对齐<br />* 同一场景下对比不同算法或不同参数<br />* 批量仿真：多场景、多参数组合自动运行，避免手工反复操作 | * 扩展性（支持协同和集群能力扩展）<br />* 可移植性：为 C 仿真/半物理仿真对齐留出口 | * 分离并沉淀仿真、算法报告、设计和分析工具<br />* 一个月的开发时间，实际投入约 0.5 人月 |
| **用户级需求** | * 支持更换不同的算法<br />* 支持添加不同的扰动（链路延迟、丢包、断链、风扰动、控制滞后、障碍物、单机故障、定位漂移等）<br />* 支持仿真结果日志记录和自动化分析<br />* 支持数据配置、期望队形、实时显示、超频运行、分段仿真、三维可视化（可选）等功能<br /> | * 扩展性：算法、扰动、仿真类型可扩展<br />* 易用性：配置、运行、查看结果方便 | * 使用者非专业人员，因此需要支持可视化的UI                   |
| **开发级需求** | * 三种仿真的数据分析工具拉齐<br />* 支持单步仿真<br />* 支持自动化测试用例和回归框架 | * 可维护性：便于迭代和排查问题                       | * 个人对python熟悉度不够<br />* 提高开发效率<br />* 仅支持3自由度仿真<br />* 不需要支持操作系统<br />* 利用ai进行开发<br />* 文档推动开发 |

# 三、系统CA阶段

## 3.1 鲁棒图

按 Jacobson/ICONIX 三要素拆分：Boundary（边界）、Control（控制）、Entity（实体）。平台包含 5 个 Control：仿真控制、模型迭代、编队算法、通信功能和加扰。
...
| Control | 加扰 | ① 持 model/comm 引用，统一管理不确定性索引 + 动态扰动；<br />② init 时把 stochastic 扰动配置 + seed 分发到 model/comm 的 set 接口；<br />③ 运行期 tick 推进动态扰动（wind / fault / link_fault）并 push 到 model/comm；<br />④ 接收 UI 经仿真控制下发的动态注入命令 |
...
本平台是单一系统，采用"切系统为子系统"的方式分割。分割结果如下：

![高层分割](./assets/高层分割.png)

> 图源：[`高层分割.drawio`](./高层分割.drawio)。
...
1. **分组标准是"变化方向一致"**：验证对象组随"换编队方法"变，仿真环境组随"换仿真形态（python/C/半物理）"变，人机交互组随"换交互方式"变。变化方向相同的节点归入同一组，便于按组替换。
...
4. **跨三种仿真的对齐点共三个**：① 验证对象组的代码（唯一被移植，要求保持"无 I/O、不持引擎引用"的纯算法形态，便于翻译为 C）；② 关键数据的日志格式（三种仿真统一落盘格式，"三种仿真的数据分析工具拉齐"由此达成）；③ 配置文件 schema。其余部分各仿真自治，C 仿真/半物理复用本文的架构边界和数据契约。
5. **离线工具边界**：报告生成和数据分析工具不参与仿真闭环，离线消费关键数据落盘文件；平台与离线工具的契约是日志格式和配置 schema。物理部署边界在半物理仿真形态中展开。
...
| 类别 | 选型 | 备注 |
| --- | --- | --- |
| 数值 | numpy | 通信丢包随机数等数值能力；复杂积分器可按模型需要引入 scipy |
...
启动方式：`python src/ui/gui/main_window.py`。启动后在界面中选择 `configs/base.json` 或其他配置文件。
...
sequenceDiagram
    ...
    loop 每个不确定性索引 N
        Bat->>CLI: ② 起进程或调用脚本（--headless --config configs/base.json --seed N）
        CLI->>Cfg: ③ 读基础配置
        Cfg-->>CLI: ④ 配置对象
...
sequenceDiagram
    ...
    SimCtrl->>Comm: ⑥ 写入本拍 outbox（含参考信号）
...
| 关键数据落盘 | 本版固定 `20 Hz`（每 `0.05 s` sim-time 一帧）；默认 `0.005 s` 基础 tick 下每 10 tick 记录一次 |
...
| **关键数据** | 仿真控制 按 `20 Hz` sim-time 节拍写，头部写入实际 不确定性索引；离线分析工具读 | 仿真过程关键变量持久化；日志格式三种仿真统一对齐，schema 见 LLD |
