...
按 Jacobson/ICONIX 三要素拆分：Boundary（边界）、Control（控制）、Entity（实体）。鲁棒图阶段 Control 节点 5 个（仿真控制、模型迭代、协调算法、节点算法、通信功能），位于 ICONIX 推荐上限内；3.4 高层分割阶段新增 1 个横切机制 Control（加扰），共 6 个。

> **关于命名**：本平台目标是支持多种编队方法（领航-跟随 / 虚拟结构 / 行为法 / 一致性 等），算法的角色分布因方法而异。因此采用中性命名：
> - **协调算法**（可选）：集中式策略中承担全局协调的算法实例。领航-跟随中在长机、虚拟结构中在地面站/虚拟节点；分布式方法（如一致性、行为法）中此节点缺省。
> - **节点算法**（多实例 ×N，N≥1）：每架飞机本地运行的算法实例。
...
| Boundary | CLI 入口 | 接收 CLI 参数（`--config` `--seed` `--output` `--headless` 等）；加载配置文件、CLI 覆盖、调用 `仿真控制.start(config)`（headless 模式启用；GUI 模式不启） |
| Control | 仿真控制 | ① 基于配置和界面初始化各算法和模型，并将拓扑/QoS 等参数下发给通信功能；<br />② 任务调度（协调算法、节点算法、模型迭代、加扰）；<br />③ 把扰动配置 + 不确定性索引整体下发给加扰；节拍触发加扰 tick；转发 UI 经控制界面下发的动态扰动注入命令；<br />④ 关键数据定时落盘；<br />⑤ 实时数据推送 |
| Control | 模型迭代 | 统一推进所有飞机的动力学积分；内部按 init 配置 + seed 实施传感器噪声 / 定位漂移 / 控制滞后；通过 inject_wind / inject_fault 接受加扰 push 的动态扰动 |
| Control | 协调算法 | 集中式策略的协调实例（可选，0 或 1 个）；运行位置依方法而定（长机 / 地面站 / 虚拟节点） |
| Control | 节点算法 | 每架飞机本地的算法实例（×N，N≥1）；含编队、制导、控制律 |
...
3.1 中 **协调算法** 与 **节点算法** 是平台对各编队方法的中性抽象。本节给出主流方法下两个抽象**具体代表什么、跑在哪、彼此怎么通信**——把"抽象"落到"实例"的映射。

（本节不是 ICONIX 意义上的 Use Case；真正的执行流程见 4.4 运行视图。）

| 编队方法 | 协调算法（位置 / 职责） | 节点算法（位置 / 职责） | 通信拓扑 | 主要消息 |
| --- | --- | --- | --- | --- |
| **领航-跟随 (Leader-Follower)** | 长机机载；航线规划 + 队形指令生成 | 长机+僚机均机载；长机自身航迹跟踪，僚机相对位置跟踪 | star（长机中心） | 长机状态 → 僚机（位置/速度/姿态） |
| **虚拟结构 (Virtual Structure)** | 地面站 / 虚拟节点；虚拟刚体位姿规划 + 几何映射到各机期望位置 | 各飞机机载；自身在虚拟刚体上对应点的位置跟踪 | star（虚拟节点中心） | 虚拟刚体状态 → 各飞机期望位置 |
| **行为法 (Behavioral)** | **缺省**（不存在） | 各飞机机载；行为合成（队形保持 + 避障 + 趋向目标） | mesh 或邻居图 | 邻居状态互发 |
| **纯一致性 (Pure Consensus)** | **缺省** | 各飞机机载；平均/状态一致性律 | 任意连通图（可时变） | 邻居一致变量（位置/速度差等） |
| **参考一致性 (Reference Consensus)** | 参考节点（虚拟领导者 / 外部参考信号源）；**仅产生参考信号**，不做集中规划 | 各飞机机载；带参考输入的一致性律 | 含参考节点的连通图 | 参考信号 + 邻居一致变量 |

### 3.3.1 关键观察

1. **协调算法可缺省，且"参考节点 ≠ 协调算法"**：纯分布式方法（行为法、纯一致性）下协调算法不存在；参考一致性下虽有"参考节点"，但它只产生参考信号、不做全局规划，**不属于传统集中式协调算法**——架构上可以视为一种特殊的"轻量协调"。平台需支持 0 个协调算法、1 个完整协调算法、1 个仅参考型协调算法 三种合法形态。
2. **协调算法的物理位置不固定**：机载（领航-跟随）/ 地面站（虚拟结构）/ 虚拟参考点（参考一致性）都可能；架构上把"算法实例"与"物理位置"解耦，由配置文件指定。
...
| 验证对象组 | 协调算法、节点算法 | **被验证、被移植对象**：python 验证可行性后翻译为 C、再上半物理，是三种仿真间唯一搬动的代码 |
...
> **验证对象组的内容**：组名取分组依据（被验证、被移植）而非内容物。组内包含三类：任务流程骨架（集结→保持→重构等阶段切换的状态机，跨编队方法通用）+ 编队方法插件（特定，被替换）+ 制导/控制律（通用底座）。"流程 vs 方法"的拆分是同一架构元素（节点算法）内部多个实现的共性抽取（模板方法/策略模式），不改变任何组间关系，属于 LLD 的类设计，架构层面不展开。注意区分：场景脚本（何时变换队形、何时注入扰动）是仿真实验编排、模拟外部指令源，属于编排组+配置文件，不属于本组。
...
包依赖与 3.4 高层分割图同构，不另绘；接口契约见下图。本节按 3.1 + 3.4 的 6 个 Control 对象，列出每个对外暴露的接口"类别"——具体 API 签名属 LLD。

接口按以下五类划分（图中各包内罗列）：

- **生命周期**：init / start / pause / resume / stop / close 等
...
- 协调算法可选：领航-跟随 / 一致性等无独立协调器时，仿真控制直接驱动节点算法，本包不实例化。
...
- 用户点"运行" → UI 调 `仿真控制.start(config)`
- 运行中：UI 可调 `pause / resume / step / stop / inject_disturbance`（step 用于 pause 态下单步推进，PA 单步仿真需求由此落地）；仿真控制 推送实时数据流给 实时显示
- 跑完：用户选择关闭 / 重跑
...
sequenceDiagram
    ...
    User->>UI: ⑥ 点"运行"
    UI->>SimCtrl: ⑦ start(config)
    Note over SimCtrl: 进入 tick 循环（见 4.4.3）<br/>运行中 UI 继续可调<br/>pause / resume / stop / inject_disturbance
```
...
- 单进程内：仿真控制 解析 CLI（CLI 覆盖配置文件）→ 调 `start(config)` → 跑完落盘 → 退出
...
sequenceDiagram
    ...
    participant Coord as 协调算法(可选)
    participant Agent as 节点算法
...
| --- | --- |
| 协调算法 | 1–10 Hz（轨迹规划尺度） |
| 节点算法 | 50–200 Hz（控制律尺度） |
| 模型迭代 | 与节点算法同步或更高（积分精度） |
| 关键数据落盘 | 10–100 Hz |
