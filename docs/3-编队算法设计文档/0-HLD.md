...
| --- | --- |
| 收发处理 | envelope 收（解析 → `ParsedInbox`）/ 发（长机：组装 `leader_nav + 队形 + 槽位` → 广播 envelope）（实现：领航-跟随收 / 发、点对点、不发…） |
...
sequenceDiagram
    ...
    Note over LE: 一个对象内：飞行流产 control + 协调流(编排 + 收发(发))产广播
...
| 2 | 任务编排 | F | `ParsedInbox.task`（来自长机广播）→ `Mode="保持"` |
...
| 步 | 阶段 | 库 | 入 → 出 |
| --- | --- | --- | --- |
| 1 | 任务编排 | F | `mission_command`（注入）→ `Mode="保持"` |
...
| 6 | 收发处理(发) | F | `(self_state, Mode)` + 静态队形 / 槽位 → 组装 `leader_nav + 队形 + 槽位` 广播 `outbox` |

> 各单元由 `step()` 按写死的固定顺序串联（动态管线重连本轮不建），不单列"执行"环节。长僚机 4–5 步完全同构，只差第 1–3 步"产 `Target`"的来源（航线 vs 槽位）。
...
| 10 | **每个提炼出的单元都是一个策略族**（抽象基类 + 多实现，同 `u`、同 `y`）：情景差异靠**族内换实现**、非散落 `if/else`；**挂接 = 实体为每个族选定一个实现**（`init` 时实例化），运行期或换方案时换实现不动实体代码。通用数学等工具函数不是 `step` 单元、不在此列；`ControlLaw` 被 `Tracking` 组合调用、由其间接选 |
