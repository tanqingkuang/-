...
> 因本模块体量大，按"一个独立子系统"来设计：**本册是模块架构（按 5 视图）**；接口/实现见 `1-Context和叶类型.md`、`2-实体组LLD.md`…，方案组合见 `5-用例-领航跟随保持.md`。

## 1. 定位与本轮范围

- **定位**：每架飞机本地一个飞行实体（×N）+ 可选非飞行协调实体（×0/1）；承载编队、跟踪、控制律。纯算法形态（无 I/O、不持引擎引用、消息驱动），便于移植到 C / 半物理；
- 现状：由于本项目为算法验证平台，为支持算法的可替换性，HLD 需要良好的架构设计；而在开发过程中需对每一类算法逐步开发和验证，因此 LLD 持续更新并维护这些算法之间的关系，本轮 LLD 主要面向《5-用例-领航跟随保持.md》并进行一定程度的预埋占位；
...
* 几个关键原则：
  ...
  * 为了解决重入问题，算法/流程单位都是类，需要对象组中的对象进行实例化；
  * 为了解决扩展性和通用性的矛盾，这里采用黑板模式+端口绑定，具体参考3-数据视图
...
| --- | --- |
| 位置解算 | 解算需要跟踪的目标位置信息 |
| 误差解算 | 根据目标位置和本地位置信息，获得位置误差 |
| 轨迹 / 编队跟踪 | 根据误差信息计算得到三轴加速度 |
...
在数据流设计的时候遇到了接口稳定性、单元扩展性和易用性的矛盾：

* 绝大部分的算法单元和流程单元都要支持不同的实现，因此这些单元需要使用策略模式对外屏蔽实现细节 —— 参考2.1的设计原则；
* 如果直接使用context让每个算法单元和流程单元直接进行读写等操作，后续单元扩展较为方便，但是由于各个单元对于context的操作细节外部不感知，因此实体选择单元进行流程编排时就产生了困难 —— 这个方案fail；
* 那么就尝试让每个单元自行说明输入输出，从而外部能够一眼明确数据流走向，从而方便实体进行组合；
  * 但是如果把输入平铺在step接口入参，则可能导致后续一个新的子算法/子流程添加时改变接口定义 —— 这个方案fail；
  * 因此就需要将输入、输出分别结构体化，从而减小算法扩展对接口稳定性的影响，但是结构体化后，又会引入实体串联各个单元时数据拼接困难 —— 这个方案fail；
...
以《5-用例-领航跟随保持.md》为例（1 个航段 + 集中式通信 + 长机僚机 + PID 控制 + 三角队形），长机与僚机复用同一套单元，但因角色不同，部分步骤为空或仅占位：

* **长机**：自身产模态、推进航线 → 算目标 → 求误差 → 跟踪 → **广播** envelope；不收消息；
* **僚机**：先**收**长机广播并解析 → 不编排、轨迹规划仅预埋占位 → 基于主机位置+队形+槽位号算目标 → 求误差 → 跟踪；不发消息。

两机唯一的耦合点是长机 ⑦ 广播的 `list[MessageEnvelope]` 喂入僚机 ① 的收消息；④⑤⑥ 两机同构，仅位置解算的输入来源不同。

图中：块间纵向箭头表示一帧内的**调用顺序**；每个有效步骤对右侧 `Context` 黑板各有一组**读（虚线）/ 写（实线）**箭头，体现单元只与黑板交互、彼此不直接传参（长机 ⑦ 发送仅读黑板、僚机 ① 收消息仅写黑板）；灰色虚线块表示该场景下为空或仅预埋占位、不读写黑板的步骤。
...
└── units/                           # L2 单元
    ...
    │   ├── pos_err_calc/            #   PosErrCalc 误差解算 → guidance.py
    │   ├── pos_track/               #   PosTrack 跟踪 → pid_compose.py
    │   ├── ctrl/                    #   Ctrl 原子控制律 → pid.py（l1/adrc 预埋）
    │   └── formation_math/          #   FormationMath 纯工具函数（非策略族）
    └── process/                     # 流程组（碰 Mode）
        ├── inbound/                 #   Inbound 收 → leader_follower.py
        ├── outbound/                #   Outbound 发 → leader_broadcast.py
        ├── formation_task/          #   FormationTask 编排 → hold.py（占位恒"保持"）
...
> 注：早期「协调算法 + 节点算法」二分遗留的 `coord/`、`node/`、`base.py` 待代码开发时清理。
...
| L2 算法组（units/algo） | L3 黑板、L4 叶类型、通用数学 | **流程组**、其它算法单元、L1 |
...
* **单元之间互不依赖**——算法/流程单元从不直接 import 彼此，只通过黑板端口间接交互（呼应 3.2）；
