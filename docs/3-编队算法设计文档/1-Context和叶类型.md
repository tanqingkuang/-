...
- 主配置使用 `formation.formation_files` 引用外部队形文件时，必须同时声明 `formation.coordinate_system = "x_forward_y_up_z_right"`，避免轴序不一致的槽位配置被静默解释。
...
@dataclass
class WayLineS:                            # 单段航段
    start: PosInEarthS = field(default_factory=PosInEarthS) # 起点
    end: PosInEarthS = field(default_factory=PosInEarthS)   # 终点
    vdCmd: float = 0.0                     # 本段地速指令，m/s
    turnSign: float = 0.0                  # +1 左转、-1 右转、0 直线
    center: PosInEarthS = field(default_factory=PosInEarthS) # 圆弧圆心

# 辅助：圆弧半径 = hypot(start.east - center.east, start.north - center.north)
...
> 只有需要**跨拍保留**或**被多个流程读写**的工作状态才放入 Context。边界 I/O（`inbox/outbox/remote`）和 `PosTrackDiagS` 由 `EntityRuntimeS` 持有；初始化配置由实体和产品保存。统一入站解析后，`leaderState`、`leaderCmd`、`cmd`、`rallyPlan` 和 `followerStates` 均写入 Context，供后续固定流程读取。完整规则见《0-HLD.md》3.3。
...
class FormContextS:                        # 编队算法上下文（跨拍常驻、原地迭代）
    ...
    leaderCmd: MotionProfS = field(default_factory=MotionProfS)   # 长机有效运动指令（统一入站写，槽位解算读）
    selfCmd: MotionProfS = field(default_factory=MotionProfS)     # 本机目标运动状态（位置解算输出）
    effectiveCmd: MotionProfS = field(default_factory=MotionProfS)# 本机限幅后有效指令（跟踪写，出站读）
    ...
    posCalcStatus: PosCalcStatusS = field(default_factory=PosCalcStatusS)       # 位置解算阶段与诊断状态
    clock: AlgorithmClockS = field(default_factory=AlgorithmClockS)             # 当前算法时间
    followerStates: list[FollowerStateS] = field(default_factory=list)          # 僚机状态回报快照
    rallyPlan: RallyPlanS = field(default_factory=RallyPlanS)                   # 公共到达时刻与圈数计划
```
