...
FLYING 每帧计算并广播 ETA：`ARC_TO_TANGENT` 包含待命圆剩余弧段、公切线段和集结圆 `T_rally → M_i` 弧段；进入直线段后改为当前点到 `T_rally` 的剩余时间加集结圆弧段。直线速度与现有控制共用 `slow_radius_m` 近场降速和最小速度托底。
...
- 长机 → 僚机：`formation.leader` 消息，含 `{cmd, slot_scale, t_ref, t_ref_valid, leader_state}`；其中 `cmd["leader"]` 携带长机有效运动指令，供僚机槽位坐标系建系
...
作用：唯一的长机广播实现，普通保持与集结共用。它把长机实际状态 `selfState`、长机有效指令 `leaderCmd`、`cmd.stage/pattern/step`、`slotScale.scale/scaleRate`、`t_ref/t_ref_valid` 打入同一条 `formation.leader` envelope；`leaderCmd` 未绑定时回退 `selfState`。普通保持场景也使用该类，只是 `slotScale` 恒为 `scale=1.0/scaleRate=0.0`，`t_ref_valid=False`，对保持编队无影响。
...
作用：唯一的长机广播入站解析实现，普通保持与集结共用。它在同一条 `formation.leader` 消息中解析 `leader_state/cmd/slot_scale/t_ref/t_ref_valid`，写入 `Context.leaderState/cmd/slotScale/rally_t_ref/rally_t_ref_valid`，并把 `cmd["leader"]` 写入对象组边界持有的 `leaderCmd` 端口；旧格式缺少 `cmd["leader"]` 时，`leaderCmd` 回退为 `leader_state`。缺少 `slot_scale` 的旧格式消息按 `scale=1.0/scaleRate=0.0` 兜底；`t_ref_valid` 缺失或非法时为 `False`，避免冷启动误切出。
...
for envelope in inbox:
    ...
    # leaderState/leaderCmd/cmd/slotScale/t_ref 必须来自同一条消息，不能跨消息拼装
    _write_motion_from_payload(payload["leader_state"], y.leaderState)
    _write_cmd_from_payload(payload["cmd"], y.cmd)
    leader_payload = payload["cmd"].get("leader") if isinstance(payload["cmd"], dict) else None
    if y.leaderCmd is not None:
        _write_motion_from_payload(leader_payload if isinstance(leader_payload, dict) else payload["leader_state"], y.leaderCmd)
    try:
...
作用：长机与僚机共用的集结位置解算单元，内部 `STANDBY → FLYING → LOITERING → EXITED` 状态机：本地待命盘旋 → 待命圆切出并沿定向公切线进入集结圆 → 沿集结圆盘旋协调到达时刻 → 从 M_i 沿任务航向切出。`FLYING` 的 `ARC_TO_TANGENT/LINE_TO_RALLY_ENTRY` 是私有子阶段，对外均保持 `FLYING`。完整算法（两圆公切线、切入触发半径、LOITERING 半径跟踪、T_ref 聚合协议）见「二、总体策略与状态机」第二步；本节只列初始化/输入端口的 API 形状：
...
作用：`SlotGeometry` 是唯一的僚机槽位几何实现。普通保持场景不绑定 `slotScale` 或使用默认 `scale=1.0/scaleRate=0.0`，输出标准槽位；集结 CATCHUP/LOOSE/COMPRESS/HOLD 绑定 `Context.slotScale`，在同一 `step()` 中对水平槽位偏置做缩放，并按 `scaleRate` 追加压缩速度前馈。COMPRESS 阶段 scale 随时间线性减小，目标位置随之平滑收敛。
...
**实现方式**：槽位查找、航迹旋转、转弯速度前馈和缩放后处理都在 `SlotGeometry.step()` 内完成，不再维护单独的缩放槽位文件。

```python
class SlotGeometry(PosCalcBase):
    def step(self, u: SlotGeometryInputS, y: PosCalcOutputS) -> None:
        # 先按 scale=1 解算标准槽位位置与速度前馈
        ...
        ...
        # 标准槽位速度给出 leaderVel + dR/dt·slot；提取旋转前馈再乘 scale，加 scaleRate 项
        ...
        # vUp 和 dVPsi 不随 scale 变化
```

`slotScale=None` 或 `scale=1.0` 且 `scaleRate=0` 时输出标准槽位（偏置无放大、无速度修正）；LOOSE 阶段 `scale=looseScale>1, scaleRate=0`，水平位置偏置放大但无额外速度项；COMPRESS 期间 `scaleRate<0` 自动添加向内的速度前馈。

测试：

- slotScale 未绑定或 scale=1.0 → pos/v 结果与标准槽位相同
- scale=2.0 → 位置偏置扩大一倍，速度前馈同步缩放
- scale 从 2.0 线性减到 1.0 → 目标水平位置平滑收敛，vEast/vNorth 同步变化，高度与 vUp 不随 scale 变化
...
self._inbound.step(self._inbound_u, self._inbound_y)          # 解析长机广播 → leaderState/leaderCmd/cmd/slotScale
...
- **端口绑定（不可遗漏）**：
  - `RallyLeaderFollower` Inbound 的 `leaderCmd` 输出 → 对象组边界持有的 `_leader_cmd`，供 `SlotGeometryInputS.leaderCmd` 使用；`slotScale`/`t_ref`/`t_ref_valid` 输出 → 每拍写入 `Context.slotScale`/`rally_t_ref`/`rally_t_ref_valid`
