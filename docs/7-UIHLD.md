...
- 正式地形采用三层融合消除有限地图的矩形硬边：配置范围内保留原分辨率高精网格，每侧追加 40 个径向采样，并按核心地图总跨度的 8 倍确定高度、颜色和切线基的视觉融合覆盖范围（即融合半径为核心跨度的 4 倍）；几何覆盖半径另设不小于 120 km 的远裁剪安全下限，若该下限更远，两者之间保持纯天际色水平地面。外围边界必须越过相机远裁剪面，不能通过把整张地形改成透明材质破坏山体对飞机、航线和尾迹的深度遮挡。
- 距离雾的起止位置只跟地图有效物理跨度关联，不随相机缩放同步后退；外围地面末端、距离雾和二维天空天际线共用同一颜色，使重置、俯视、侧视及手工缩远后都不出现棋盘状边界或新的色阶。
