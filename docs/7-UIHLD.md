...
- 启用障碍不再渲染为固定高度的红色圆柱或方盒；圆形与多边形（含安全间距）按真实水平轮廓混入地形顶点色，以低饱和铁锈红表达风险，同时保留地形起伏、光照和贴图细节。禁用障碍必须同步清除对应着色。
- 航线复用连续线带几何，但不绑定展示末端，始终保持静态路径语义；场景没有障碍库时，布局文件中的兼容风险峰仍可使用风险线和缓冲圈提示。
