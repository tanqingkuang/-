...
- 中央实时显示区：俯视图、侧视图、图例、网格开关、自动居中、重置视图、全屏显示。
- 右侧状态区：节点状态表、链路状态表。
...
- 手动视图操作：俯视图和侧视图都支持中键拖动平移、左键拖拽矩形区域放大、滚轮滚动连续缩放、左键双击或“重置视图”按钮恢复默认视图。侧视图的横向操作作用于共享横向视野，并同步刷新俯视图；侧视图的高度方向操作只调整侧视图高度轴，不改变俯视图纵向位置。用户手动操作视图时关闭自动居中，避免后续刷新覆盖手动视角。
...
- 与俯视图共享横向视野，俯视图缩放和平移时侧视图同步；侧视图网格竖线按俯视图世界坐标网格绘制。
- 支持中键拖动平移、左键框选区域放大、滚轮连续缩放、左键双击和按钮重置视图；侧视图横向操作同步更新俯视图横向视野，高度方向操作只缩放或平移高度轴。
- 展示节点高度、参考高度线和历史轨迹。
- 不单独控制仿真状态。
