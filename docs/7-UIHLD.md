...
- 3D 态势补间：飞机与尾迹活动末段共用同一个 90 ms 展示进度。历史大网格只固化上一条展示消息已经覆盖的稳定队列前缀，本轮批量样本延后一条展示消息原子追加；活动末段从已固化队尾连接飞机共同补间位置。展示坐标不入队，不增加尾迹容量，也不移动中间历史中心线。
...
- 正常数据刷新约为 100 ms，展示补间为 90 ms。偶发提前到帧进入容量为 2 的展示消息队列并按序消费；极端积压时先共同完成当前目标，再处理最老消息，不能丢弃增量尾迹消息。机型切换或地形就绪造成的同一时刻 payload 重推同样进入该队列，上一条补间完成后才可固化其观察到的队尾。
