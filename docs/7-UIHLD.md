...
- 中央实时显示区：俯视图、侧视图、图例、网格开关、自动居中、航段锁定、视角滑条、重置视图、全屏显示。
- 右侧状态区：节点状态表、链路状态表。
...
- 航段锁定时，“视角”滑条禁止手动拖动，但随当前航段自动更新。视角定义为观察方向：`0°` 表示面朝正北，横轴为东向距离；`90°` 表示面朝正东，横轴为南向距离。
- 关闭航段锁定或当前快照没有可锁定航段时，“视角”滑条允许在 `0..360°` 范围内手动拖动，侧视图横轴按该观察方向投影得到。
