...
- 2D 尾迹绘制：分块 `QPainterPath` 只允许用画笔描边，绘制前必须显式使用 `NoBrush` 并恢复调用方状态；禁止让航点、机体或障碍遗留画刷把开放折线隐式闭合填充。
- 3D 态势补间：飞机与尾迹活动末段共用同一个 90 ms 展示进度。真实采样点立即进入有界逻辑队列；渲染时历史大网格只提交到倒数第二个真实点，最新真实点作为活动末段目标。展示补间坐标不入队，不增加尾迹容量，也不移动中间历史中心线。
...
3D 态势展示三维地形、航线、障碍、飞机和连续尾迹：

- 数据层仍以 ENU 真实采样点和稳定序号为权威来源，3D 显示层只做 Quick3D 轴映射与展示补间。
- 每次快照到达时，飞机目标位置和尾迹真实末点同时更新；两者通过同一 `presentationProgress` 从上一展示位置移动到新目标，禁止各自维护独立动画时钟。
- 历史尾迹使用增量 `TrailRibbonGeometry`，只在真实快照到达时追加、弹头或修正新折角；补间期间其顶点和索引缓冲必须保持不变。
- 活动末段使用独立 `TrailTipGeometry`，固定为 6 个顶点和 9 个索引；其中两个三角形绘制末段，至多一个 bevel 三角形闭合与历史平头端帽的转角。60 Hz 更新量恒定，不随历史点数增长。
- 正常数据刷新约为 100 ms，展示补间为 90 ms。偶发提前到帧进入容量为 2 的展示消息队列并按序消费；极端积压时先共同完成当前目标，再处理最老消息，不能丢弃增量尾迹消息。
- 相机重置只读取最新 payload 的相机字段，禁止调用整帧场景更新；否则会绕过展示消息队列并破坏尾迹增量游标顺序。
- 航线、风险线和缓冲圈复用连续线带几何，但不绑定展示末端，始终保持静态路径语义。
