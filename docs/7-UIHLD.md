...
- 3D 态势补间：飞机与尾迹最新段共用同一个 90 ms 展示进度。真实采样点立即进入有界历史队列，但最新段的可见末端只到飞机当前展示位置；补间完成后该真实点自然转为稳定历史点。展示补间坐标不入队，不增加尾迹容量，也不移动中间历史中心线。
...
- 连续尾迹 mesh 只临时覆写最后一个物理段槽及其相邻接头；补间期间不重建全长网格，不修改真实点 deque、generation、序号或容量。
