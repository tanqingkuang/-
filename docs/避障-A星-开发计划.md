...
> 状态：**已评审，实施中**。方案已定稿；按"第 6 节 分步开发计划"逐步实施，每完成一大步由作者做代码检查 + 手动测试。
> 进度：**步骤 1（UI 障碍显示 + 勾选）已落地**（界面提前到首步），规划后端（A\* + 出口翻译 + 可飞性校验，步骤 2–5）待续。
...
- **不做地图鼠标绘制**；障碍离线写 JSON，界面只做"勾选 + 生成航线 + 预览确认 + 采用并仿真"（§6.1），规划与仿真解耦。
...
"avoidance": {
  ...
  "obstacles": [
    { "id": "C1", "type": "circle", "enabled": true,
      "center": {"east_m": 600, "north_m": 300}, "radius_m": 80 },
    { "id": "R1", "type": "rect", "enabled": false,
      "min": {"east_m": 900, "north_m": 100}, "max": {"east_m": 1100, "north_m": 500} }
  ]
}
```
...
- 每个障碍带 `id`（界面列表显示 / 勾选用）与 `enabled`（默认勾选状态）。JSON 是**障碍库**，界面再从中**勾选本次启用的子集**——只对勾选项规划，未勾选的不参与（见 §6.1）。
...
接入采用**规划与仿真解耦**：`sim_control.py` 暴露一个 `plan_route(选中障碍) → 预览 RouteS`（或失败原因）接口，由界面"生成航线"按钮**显式触发**，不在加载配置时自动跑；预览经人工确认后才"采用"为长机航线、再仿真（详见 §6.1）。障碍是**空间持久对象**，单走此接口，**不复用 `DisturbanceCommand`**（那是带时长的扰动）。
...
2. 点 **【⟳ 生成航线】** → 后端 `plan_route(选中障碍)` 跑 A\* + 可飞性校验（**不进仿真**）。
3. 俯视图**预览**：红=障碍、橙虚线=膨胀、绿虚线=规划航线；失败则左面板显示 **`ERR_AVOID_*` 具体原因**，不出航线。
4. 预览满意 → 点 **【采用并播放】** 才按这条航线仿真；不满意就改 JSON / 改勾选，重新"生成航线"。

> 预览航线先**只在内存中**，确认即用；存回 JSON 留作后续增强。
...
### 步骤 5：控制器接入 —— `plan_route` 接口（规划/仿真解耦）
- **输入**：长机原始航线 + 选中的 `ObstacleS[]` + `avoidance` 参数。
- **输出**：`sim_control` 暴露 `plan_route(选中障碍) → 预览 RouteS`；失败按 **§9** 返回 `ERR_AVOID_*` 原因码 + 定位（不只报"规划失败"）。确认后才把预览 `RouteS` "采用"为长机航线。
- **现象**：调用 `plan_route` 得到可绕障航线（暂不自动跑仿真）；采用后长机绕障、僚机跟随。
- **测试**：端到端——选定障碍后规划航迹始终在障碍外；空选 / `enabled=false` 等价现状；无解返回原因码、不崩；采用后仿真正常结束。
- **手测**：脚本调用 `plan_route` 核对航线；采用后跑一遍确认绕障。

### 步骤 6：界面 —— 障碍勾选 + 生成航线 + 预览确认（见 §6.1）
- **输入**：JSON 的 `obstacles[]`、`plan_route` 结果。
- **输出**：左面板"避障"分组（障碍勾选列表 + 【⟳ 生成航线】+ 状态）；俯视图叠加障碍（圆 / 矩形）+ 膨胀圈 + 绿色**预览**航线；【采用并播放】确认后才仿真；失败在面板显示 `ERR_AVOID_*`。
- **现象**：勾选 → 生成 → 预览绕障 → 确认 → 仿真，全程肉眼可控。
- **测试**：GUI 渲染与交互（勾选变更、生成、预览、失败提示、采用并播放）；缩放平移不崩。
- **手测**：改勾选重生成、构造失败场景看原因提示、目视航线绕行一致。
