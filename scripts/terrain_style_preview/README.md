...
- `--grid-size`：高度场采样边长，默认 `641`。默认轴向为中心高分辨率、外围低分辨率；核心 `24km x 24km` 区域约 `46.9m` 一个采样点。
...
- 预览画布为 `50km x 50km`，核心 `24km x 24km` 走廊使用高分辨率采样，外围远山使用同一张 mesh 的低密度裙边延伸出画。
- 地形共用同一份高度几何：8 座主峰布置在蜿蜒航线走廊两侧，主峰相对高度约 `2300m~3000m`。
- 山体由椭圆主峰、ridged fBm、domain warping、方向性沟壑、外围连接丘陵和走廊内 `300m~600m` 小山头叠加生成。
- 风格 A 的地表顶点色由海拔和坡度共同决定，风险区叠加暗红半透明面片、红橙发光线框和淡青色安全缓冲虚线。
