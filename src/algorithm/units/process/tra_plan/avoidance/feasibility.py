...
3. 直线触障：最终直线腿采样后落入真实障碍 → ERR_AVOID_STRAIGHT_HITS_OBSTACLE。
4. 圆弧触障：拐点圆弧（向外凸）采样后落入真实障碍 → ERR_AVOID_ARC_HITS_OBSTACLE。
...
ERR_STRAIGHT_HITS_OBSTACLE = "ERR_AVOID_STRAIGHT_HITS_OBSTACLE"
...
# 直线腿也在这里做全障碍复核，避免上游 A* 候选障碍预筛漏掉绕行路径上的障碍。
# 直线触障默认使用真实障碍 clearance=0；额外保守量可由 arc_clearance 一并传入。
...
def _arc_sample_points(arc: tuple, radius: float, step: float) -> list[Point]:
    ...
    line = WayLineS(
        start=PosInEarthS(t1.east, t1.north, t1.h),
        end=PosInEarthS(t2.east, t2.north, t2.h),
        turnSign=turn_sign,
...
def _line_hit_obstacle(
    ...
    """线段 start→end 连续采样后的首个触障 id；无触障返回 None。"""
    length = hypot(end[0] - start[0], end[1] - start[1])
    # 长度为 0 的退化腿仍检查端点，避免重合点正好落在障碍内。
    steps = max(1, ceil(length / sample_step))
    for index in range(steps + 1):
        # 采样含两端点，能同时兜住端点落障碍和中段穿障两类问题。
        ratio = index / steps
        east = start[0] + (end[0] - start[0]) * ratio
        north = start[1] + (end[1] - start[1]) * ratio
        obstacle_id = _hit_obstacle(obstacles, east, north, clearance)
...
def check_feasibility(
    ...
    *,
    ...
    # 最终直线腿按全量障碍复核，兜住 A* 局部候选集或人工折线可能遗漏的线段触障。
    if obstacles:
        for i in range(n - 1):
            # 使用全量 obstacles，而不是 A* 预筛候选集；这是本函数作为最终兜底的关键。
            obstacle_id = _line_hit_obstacle(points[i], points[i + 1], obstacles, arc_clearance, sample_step)
            if obstacle_id is not None:
                return FeasibilityResult(
                    ok=False,
                    code=ERR_STRAIGHT_HITS_OBSTACLE,
                    detail=f"腿 {i}（拐点 {i}→{i + 1}）触及障碍 {obstacle_id}",
                    leg_index=i,
