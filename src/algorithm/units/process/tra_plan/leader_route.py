...
def waypoint_inputs_to_waylines(inputs: list[WayPointInputS]) -> list[WayLineS]:
        ...
        if wpi.turnSign != 0.0:
            ...
            if arc is not None:
                ...
                if tangent_in <= in_leg + 1e-9 and tangent_out <= out_leg + 1e-9:
                    nodes.append(WayPointInputS(pos=t1, vdCmd=inputs[i - 1].vdCmd, turnSign=turn_sign, center=center))
                    nodes.append(WayPointInputS(pos=t2, vdCmd=wpi.vdCmd))
                else:
                    nodes.append(wpi)
            else:
                nodes.append(wpi)
    ...
    return [
        WayLineS(
            ...
            vdCmd=nodes[j].vdCmd,
            turnSign=nodes[j].turnSign,
            center=nodes[j].center,
        )
...
def _turn_radius_m(line: WayLineS) -> float:
    ...
    speed = max(0.0, line.vdCmd)  # 用航段指令速度，钳到非负
