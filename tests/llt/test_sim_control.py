...
class SimulationControllerTests(unittest.TestCase):
    ...
    def test_route_waypoints_build_continuous_segments(self) -> None:
        ...
        self.assertAlmostEqual(lines[0].end.east, 100.0)
        self.assertAlmostEqual(lines[1].start.east, 100.0)
        self.assertAlmostEqual(lines[1].end.north, 80.0)
        self.assertTrue(all(line.vdCmd == 12.0 for line in lines))
    ...
    def test_route_segments_preserve_per_segment_speed(self) -> None:
        ...
        self.assertEqual([line.vdCmd for line in lines], [10.0, 20.0])
    ...
    def test_route_waypoints_radius_inserts_tangent_arc(self) -> None:
        ...
        self.assertEqual(leg_in.turnSign, 0.0)
        self.assertAlmostEqual(leg_in.end.east, 1600.0)
        self.assertAlmostEqual(leg_in.end.north, 0.0)
        self.assertAlmostEqual(arc_radius(arc), 400.0)
        self.assertAlmostEqual(arc.turnSign, 1.0)  # 左转/逆时针
        self.assertAlmostEqual(arc.start.east, 1600.0)
        self.assertAlmostEqual(arc.end.east, 2000.0)
        self.assertAlmostEqual(arc.end.north, 400.0)
        self.assertAlmostEqual(arc.center.east, 1600.0)
        self.assertAlmostEqual(arc.center.north, 400.0)
        self.assertEqual(leg_out.turnSign, 0.0)
        self.assertAlmostEqual(leg_out.start.north, 400.0)
        self.assertAlmostEqual(leg_out.end.north, 2000.0)
    ...
    def test_arc_route_cross_track_error_uses_radial_distance_not_chord(self) -> None:
        ...
        on_arc_midpoint = _aircraft_state(
            "A01",
            arc.center.east + arc_radius(arc) / math.sqrt(2.0),
            arc.center.north - arc_radius(arc) / math.sqrt(2.0),
            1000.0,
        )
    ...
    def test_arc_route_cross_track_error_positive_to_track_right_side(self) -> None:
        ...
        left_outside = _aircraft_state(
            "A01",
            left_arc.center.east + (arc_radius(left_arc) + 10.0) / math.sqrt(2.0),
            left_arc.center.north - (arc_radius(left_arc) + 10.0) / math.sqrt(2.0),
            1000.0,
        )
        ...
        right_inside = _aircraft_state(
            "A01",
            right_arc.center.east + (arc_radius(right_arc) - 10.0) / math.sqrt(2.0),
            right_arc.center.north + (arc_radius(right_arc) - 10.0) / math.sqrt(2.0),
            1000.0,
        )
    ...
    def test_display_route_keeps_original_segments_without_arc(self) -> None:
        ...
        self.assertAlmostEqual(display[0].end.east, 2000.0)
        self.assertAlmostEqual(display[0].end.north, 0.0)
        self.assertAlmostEqual(display[1].end.north, 2000.0)
...
class NodeAlgorithmResetTests(unittest.TestCase):
    ...
    def test_reset_clears_rally_completed_flag(self) -> None:
        ...
        route = [
            WayPointInputS(pos=P(east=0.0, north=0.0, h=500.0), vdCmd=20.0),
            WayPointInputS(pos=P(east=100.0, north=0.0, h=500.0), vdCmd=20.0),
            WayPointInputS(pos=P(east=200.0, north=0.0, h=500.0), vdCmd=20.0),
        ]
        node = _NodeAlgorithm(
            node_id="R01",
            role="rally_leader",
